...
int adventurer_func (int drawntreasure, struct gameState *state, int currentPlayer, int cardDrawn) {
	int z = 0;
	struct myStruct **temphand[1] = {malloc(sizeof(struct myStruct *))}; 
	...
	if (cardDrawn == copper || cardDrawn == silver || cardDrawn == gold)
	  ...
	  state->handCount[currentPlayer]++; //this should just remove the top card (the most recently drawn one). Adjusted to increment instead of decrement (TK)
	  z++;
	}
      }
      while(z-1>=0){
...
int village_func (int currentPlayer, struct gameState *state, int handPos) {
      ...
      //+2 Actions
      state->numActions = state->numActions + 5;  // adjusted to add 5 actions. 
...
int remodel_func (int currentPlayer, struct gameState *state, int choice1, int choice2, int handPos) {
...
int outpost_func (struct gameState *state, int currentPlayer, int handPos) {
      ...
      //discard card
      discardCard(handPos - 1, currentPlayer, state, 0); // discard a DIFFERENT card than the outpost card getting played. 
      return 0;
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
 	  adventurer_func(drawntreasure, state, currentPlayer, cardDrawn); 
    ...
    case remodel:
	  remodel_func(currentPlayer, state, choice1, choice2, handPos); 

    case smithy:
      smithy_func(currentPlayer, state, handPos); 
		
    case village:
 	  village_func(currentPlayer, state, handPos); 
    ...
    case outpost:
	  outpost_func(state, currentPlayer, handPos); 
